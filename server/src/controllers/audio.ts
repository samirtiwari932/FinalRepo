...
export const createAudio: RequestHandler = async (req: RequestWithFiles, res) => {
...
export const updateAudio: RequestHandler = async (req: RequestWithFiles, res) => {
  const {title, about, category} = req.body;
  const poster = req.files?.poster;
...
export const getLatestUploads: RequestHandler = async (req, res) => {
  const list = await Audio.find().sort("-createdAt").limit(5).populate<PopulateFavList>("owner");
