...
export const getHistories: RequestHandler = async (req, res) => {
  const {limit = "5", pageNo = "0"} = req.query as paginationQuery;
  const histories = await History.aggregate([
    {$match: {owner: req.user.id}},
    {
      $project: {
        all: {
          //can use anyname as test and so on
          $slice: ["$all", parseInt(limit) * parseInt(pageNo), parseInt(limit)],
        },
      },
    },
    {$unwind: "$all"}, //unwind the above if name is set to test it gonna unwind the test
    {
      $lookup: {
        from: "audios",
