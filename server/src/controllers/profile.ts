// import {getIsFollowing} from "./profile";
...
import {getUserPreviousHistories} from "#/utils/helper";
...
export const getUploads: RequestHandler = async (req, res) => {
  const {pageNo = "0", limit} = req.query as paginationQuery;
...
export const getPublicUploads: RequestHandler = async (req, res) => {
  ...
  const data = await Audio.find({owner: profileId})
    ...
    .limit(parseInt(limit))
    .sort("-createdAt")
    .populate<AudioDocument<{name: string; _id: string}>>("owner");
...
export const getRecommendByProfile: RequestHandler = async (req, res) => {
  ...
  if (user) {
    ...
    const category = await getUserPreviousHistories(req);
...
export const getPlayListAudios: RequestHandler = async (req, res) => {
  ...
  if (!playlistResult) {
    const [autoplaylistResult] = await AutoGeneratedPlayList.aggregate(aggregatationLogic);
